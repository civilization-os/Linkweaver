...
async function main() {
    ...
    await store.init();
    // Setup MCP Server
    const server = new Server({ name: 'ReqFlow MCP TS', version: '1.0.0' }, { capabilities: { tools: {} } });
    ...
    app.get('*', (req, res) => {
        res.sendFile(path.join(frontendPath, 'index.html'), err => {
            if (err)
                res.status(404).send('Frontend not built yet. Run npm run build in web directory.');
        });
    });
